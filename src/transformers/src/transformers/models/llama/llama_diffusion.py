    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # # drop hidden states according to attention score in the AGG_LAYER
                # if USE_FAST_V and TOKEN_DROP:
                #     if idx<AGG_LAYER:
                #         pass
                #     elif idx==AGG_LAYER:
                #         last_layer_attention = layer_outputs[1]
                ...
                #         # get the indexs of the top ATTENTION_RANK tokens
                #         top_attention_rank_index = last_layer_attention_avg_last_tok_image.topk(ATTENTION_RANK).indices + SYS_LENGTH
                #         # drop the hidden states of the top tokens
                #         hidden_states[:,top_attention_rank_index,:] = 0
                ...
                #         new_attention_mask, (batch_size, seq_length), inputs_embeds, past_key_values_length
                #     )
                #     # set the index in the attention mask to be -65504.

                #     # shape
                #     max_x,max_y = new_attention_mask[0,0,:,:].shape

                #     # mask = torch.zeros_like(new_attention_mask, dtype=torch.bool, device=inputs_embeds.device)
                    
                #     tri_binf = ~torch.triu(torch.ones((max_y-8,max_y-8), dtype=torch.bool, device=inputs_embeds.device), diagonal=1).bool()

                #     streaming_new_attention_mask = torch.zeros_like(tri_binf, dtype=torch.float, device=inputs_embeds.device)

                #     # fill mask with -inf

                #     streaming_new_attention_mask = streaming_new_attention_mask.masked_fill(tri_binf, torch.finfo(attention_mask.dtype).min)

                #     if max_x ==1 :
                #         new_attention_mask = attention_mask
                #     else:
                #         new_attention_mask[0,0,8:,4:max_y-4] = streaming_new_attention_mask
